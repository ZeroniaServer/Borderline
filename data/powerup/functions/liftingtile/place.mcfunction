...
execute as @e[type=marker,tag=current,tag=lifting] if entity @s[tag=exploding] at @s as @e[type=armor_stand,tag=tnt,sort=nearest,limit=1] at @s run tp @s ~ ~2 ~
execute as @e[type=marker,tag=current,tag=lifting] if entity @s[tag=portal] at @s as @e[type=armor_stand,tag=anchor,sort=nearest,limit=1] at @s run tp @s ~ ~2 ~
