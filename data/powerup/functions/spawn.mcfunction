tag @e[type=marker,tag=square,limit=1,sort=random,tag=!selected] add spawnLocation
execute as @e[type=marker,tag=spawnLocation,limit=1] at @s run loot spawn ~ ~1 ~ loot powerup:items
execute as @e[type=marker,tag=spawnLocation,limit=1] at @s run summon firework_rocket ~ ~1.4 ~ {LifeTime:0,FireworksItem:{id:firework_rocket,Count:1,tag:{Fireworks:{Flight:0,Explosions:[{Type:0,Flicker:0,Trail:0,Colors:[I;15790320],FadeColors:[I;1973019]}]}}}}
