...
execute as @a[gamemode=spectator,scores={Lives=1..}] at @s run tp @s @s
execute as @a[gamemode=spectator,scores={Lives=1..}] at @e[type=marker,tag=square,tag=!selected,tag=!fallen,tag=!falling,limit=1,sort=random] run tp @s ~ ~2 ~
